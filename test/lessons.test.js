...
import {
  ...
  RENDER_OBJECTS, ROOM, STRUCTURES, STRUCTURE_CROPS, TARGET_WORDS, TUTORIAL_STAGE, VOCABULARY, attemptWaterTarget, buildFlashcards,
  createJournal, createTutorialSession, getConfirmationReadiness, getLearningState, getWaterTaskReadiness, grantItem, matchesWaterHypothesis,
...
test("the browser lesson may expose six characters but requires only water", () => {
  assert.deepEqual(TARGET_WORDS, ["你", "我", "他", "水", "是", "誰"]);
  assert.deepEqual(Object.keys(VOCABULARY), TARGET_WORDS); assert.equal(BROWSER_CURRICULUM.requiredTutorialWord, "water"); assert.equal(BROWSER_CURRICULUM.taughtTransferVerb, false); assert.equal("給" in VOCABULARY, false);
  ...
  assert.doesNotMatch(source, /invocation-panel|give \+ me|水給我|給我水|understanding-check|challenge-start|STAGE_HOSTS|PRONOUNS|IDENTITY/i);
});

test("water appears in three natural contexts without curriculum counters", () => {
  const contexts = new Set();
  for (const entity of ENTITIES) for (const dialogue of [...entity.lines, ...(entity.resolvedLines ?? [])]) if (dialogue.tokens.includes("水")) contexts.add(`${entity.id}:${dialogue.id}`);
  assert.ok(contexts.has("notice-board:board-water")); assert.ok(contexts.has("water-jar:jar-water")); assert.ok(contexts.has("thirsty-traveller:traveller-water")); assert.ok(contexts.size >= 3);
});
...
test("repeating one automatically recorded occurrence never fakes independent evidence", () => {
...
test("hypotheses become flashcards only after a world action supports them", () => {
  let journal = observe(createJournal(), "水", "jar:water"); journal = observe(journal, "水", "traveller:water"); journal = setGuess(journal, "water", "a bowl"); journal = setGuess(journal, "water", "drinking water", 3); journal = setConfidence(journal, "water", CONFIDENCE.PROBABLE); journal = setConfirmed(journal, "water", true);
  ...
  assert.deepEqual(journal.entries.water.revisions, [{ from: "a bowl", to: "drinking water", timestamp: 3 }]); assert.equal(journal.entries.water.worldVerified, true); assert.equal(journal.entries.water.confidence, CONFIDENCE.CONFIDENT); assert.equal("correct" in journal.entries.water, false); assert.deepEqual(buildFlashcards(journal).map(card => card.id), ["water"]);
});

test("water alone unlocks the tutorial consequence without pronoun mastery", () => {
...
test("displayed target words auto-record and clicking only opens their evidence", () => {
  ...
  assert.match(main, /recordDisplayedLine\(line\);/); assert.match(main, /for \(const tokenText of new Set\(line\.tokens/); assert.match(main, /button\.addEventListener\("click", \(\) => openNotebook\(vocab\.id\)\)/); assert.doesNotMatch(main, /button\.addEventListener\("click"[^\n]*recordEvidence/); assert.match(html, /recorded automatically/i);
  assert.match(main, /inputField\.addEventListener\("input"/); assert.match(main, /recordRevision: false/); assert.match(main, /inputField\.addEventListener\("blur"/);
  assert.match(main, /state\.current === GAME_STATE\.DIALOGUE\) refreshDialogueActions\(\)/);
});
